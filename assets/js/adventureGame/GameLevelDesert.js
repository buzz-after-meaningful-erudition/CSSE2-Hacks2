...
import Block from './block.js'; // <-- Make sure this exists!
...
class GameLevelDesert {
  constructor(gameEnv) {
    ...
    // Setup falling block (canvas-based)
    const block = new Block(gameEnv);

    // Game loop
    const ctx = gameEnv.context;
    const loop = () => {
      ctx.clearRect(0, 0, width, height);

      // Optional background fill
      ctx.fillStyle = '#fcebb3'; // sandy beige
      ctx.fillRect(0, 0, width, height);

      // Update and render the block
      block.update();
      block.render();
    ...
    // Object classes used in this level
    this.classes = [
